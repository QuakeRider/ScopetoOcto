...
- `plotting.py` - Diagnostic plotting and summary statistics
...
To generate diagnostic plots from a completed download:
...
download_chunk_size: "month" # Time window per API call: day, month, or year (default: month)
```
...
- `download_chunk_size`: Time window fetched in a single API call per station. Options: `day`, `month` (default), or `year`. Larger chunks mean fewer API round-trips and faster downloads. If a chunk response hits the 10,000-pick limit, the script automatically retries at the next finer granularity (`year → month → day`).
...
│   ├── per_station_figures.pdf
│   ├── stations/              # Per-station PNG figures
│   └── summary/               # Summary PNG/PDF figures and heatmaps
└── summaries/                 # Generated by --plot
    ├── station_summary.csv
    ├── network_summary.csv
    └── summary_report.txt
```
...
After a download completes, generate diagnostic figures and summary statistics with:

```bash
python workflow.py --plot ./quakescope_output
```

This reads the pick files from the output directory and produces:

**Per-station figures** (saved as individual PNGs in `plots/stations/` and combined into `plots/per_station_figures.pdf`):
- Daily pick counts (P / S / total)
- Daily P/S ratio
- Pick confidence distribution (P vs S histograms)
- Hourly pick-rate heatmap
- Cumulative picks over time
- Amplitude distribution (log scale)
- Pick confidence scatter over time
- Station summary statistics panel

**Summary figures** (saved to `plots/summary/`):
- *Fig 1 — Geographic*: Station map coloured by pick count, active-days bar chart, pick-density hexbin
- *Fig 2 — Heatmaps*: Network-aggregated, all-station combined, and per-network heatmaps (split by year for multi-year downloads)
- *Fig 3 — Temporal/statistical*: Daily time series, P/S ratio, confidence distribution, network comparison, top-station breakdown

**Text and CSV summaries** (saved to `summaries/`):
- `station_summary.csv` — per-station statistics
- `network_summary.csv` — per-network aggregates
- `summary_report.txt` — human-readable tables of top/bottom 20 stations

Plotting requires `matplotlib`, `cartopy`, and `pyproj`. Install them with:
...
- The script automatically falls back to a finer chunk granularity (`year → month → day`) when the limit is hit — a warning is printed each time this occurs
- If you still see warnings at day granularity, increase `min_score` to reduce picks or filter by a specific channel type
